import requests
from bs4 import BeautifulSoup
import pandas as pd

url = "https://m.blog.naver.com/PostView.nhn?blogId=g2s_academy&logNo=220191710236&proxyReferer=https%3A%2F%2Fwww.google.com%2F"

df_gis_url = url

header = {'User-Agent' : 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_1) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 Safari/605.1.15'}

for i in range(0, 10):
  page = requests.get(df_gis_url[i], headers=header)
  soup = BeautifulSoup(page.content, 'lxml')
  divs = soup.findAll("div", {"class": "바탕글"})

  for div in divs:
    inner_text = div.text
    strings = inner_text.split("\n")
    print (strings[0])

  ps = soup.findAll('p')

  for p in ps:
    inner_text = p.text
    strings = inner_text.split("\n")
    print (strings[0])
